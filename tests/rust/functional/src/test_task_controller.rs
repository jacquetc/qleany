...
fn test_update_string_fields() {
    ...
    let mut dto = task_controller::get(&ctx.db, &id).unwrap().unwrap();
    dto.title = "NewTitle".into();
    dto.content = "NewContent".into();
    let updated = task_controller::update(&ctx.db, &ctx.hub, &mut ctx.undo, None, &dto).unwrap();
...
fn test_update_enum_field() {
    ...
    let mut dto = task_controller::get(&ctx.db, &id).unwrap().unwrap();
    dto.difficulty = common::entities::TaskDifficulty::Expert;
    let updated = task_controller::update(&ctx.db, &ctx.hub, &mut ctx.undo, None, &dto).unwrap();
