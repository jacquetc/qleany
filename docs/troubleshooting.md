...
Qleany generates files based on what's defined in your manifest. If an entity is missing its controller or repository, check that `allow_direct_access` isn't set to `false` for that entity. Entities with `allow_direct_access: false` don't generate files in the `direct_access/` folder — this is intentional for base classes like `EntityBase`.
...
For Rust, regenerate these files together: `common/event.rs`, `common/entities.rs`, `common/direct_access/repository_factory.rs`, `common/direct_access.rs`, and `direct_access/lib.rs`.
...
This usually happens when your manifest has inconsistent relationship definitions. For example, if Entity A declares a `one_to_many` relationship to Entity B, but Entity B doesn't exist or has a conflicting back-reference, the generated code may have type mismatches.

Review your relationship definitions. Every `many_to_one` back-reference should point to an entity that actually owns the relationship with `one_to_many` or `ordered_one_to_many`. Every `many_to_many` relationship should reference an entity that exists.
...
If undo is enabled but still doesn't work, verify the undo system is properly initialized. In C++/Qt, the UndoRedoSystem singleton must be instantiated before any undoable operations. In Rust, the command stack must be created and passed to controllers.

### Redo recreates entities with wrong IDs

This can happen if your undo implementation doesn't properly store and restore entity IDs. The generated scaffolding stores the created ID during `execute()` and uses it during `undo()`. If you've modified the generated command code, ensure you're preserving IDs correctly.

Another cause is concurrent modifications. If another operation modifies the database between undo and redo, the redo may find the expected state has changed. Consider whether your application needs optimistic locking or conflict detection.
...
If you expected the internal database to persist, review the [Generated Infrastructure](generated-code.md#ephemeral-database-pattern) documentation. The pattern is: load file → populate internal database → work → save to file.
...
Also verify you're not holding onto old entity instances after they've been updated. Fetch fresh data from the repository after any operation that might have modified it.
...
If events are firing but the model doesn't update, ensure the model instance is still alive. Creating a new model instance on each navigation discards the old subscriptions. Declare models at component level, not inside functions.
...
The generated Rust code uses specific borrowing patterns. If you've modified the generated code and introduced borrow checker errors, review whether you're holding references across await points or trying to mutate while borrowing.

The generated repositories take `&mut self` for write operations and `&self` for reads. Make sure you're not trying to read and write simultaneously through the same repository instance.
...
Also check that you're actually polling the receiver. In synchronous code, call `rx.recv()` or `rx.try_recv()`. If using async, ensure the receiver task is being polled.
...
Certain relationship configurations are invalid and rejected at parse time. For example, `strong: true` is only valid on `one_to_one`, `one_to_many`, and `ordered_one_to_many` relationships — not on `many_to_one` or `many_to_many`. The `required` flag only applies to `one_to_one` and `many_to_one`.
...
If you edited the manifest but regeneration produces the same output, verify you saved the manifest file. In the UI, the header shows "Save Manifest" when there are unsaved changes. In CLI mode, you're editing the file directly, so this isn't an issue.

Also check that you're regenerating the right files. If you changed an entity's fields but only regenerated the controller, the entity struct file still has the old fields.
