...
from qleany_.common.direct_access.common.database.sqlite_db_table_group import (
    SqliteDbTableGroup,
)
from qleany_.common.direct_access.field.i_field_db_table_group import IFieldDbTableGroup
from qleany_.common.entities.entity_enums import RelationshipInfo
from qleany_.common.entities.field import Field


class FieldDbTableGroup(IFieldDbTableGroup):
    def __init__(self, db_connection: IDbConnection):
        super().__init__()
        self._sqlite_db_table_group = SqliteDbTableGroup(
            Field, db_connection.connection()
        )

    def get(self, ids: Sequence[int]) -> Sequence[Field]:
        return cast(Sequence[Field], self._sqlite_db_table_group.get(ids))

    def get_all(self) -> Sequence[Field]:
        return cast(Sequence[Field], self._sqlite_db_table_group.get_all())
    ...
    def create(self, entities: Sequence[Field]) -> Sequence[Field]:
        return cast(Sequence[Field], self._sqlite_db_table_group.create(entities))

    def update(self, entities: Sequence[Field]) -> Sequence[Field]:
        return cast(Sequence[Field], self._sqlite_db_table_group.update(entities))
