...
use super::{
    ...
    units_of_work::{CarReadUoWFactory, CarWriteUoWFactory},
};
...
pub fn create_orphan(
    ...
    let uow_factory = CarWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::Car = entity.into();
...
pub fn create_orphan_multi(
    ...
    let uow_factory = CarWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::Car> = entities.iter().map(|dto| dto.into()).collect();
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<CarDto>> {
    let uow_factory = CarReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<CarDto>> {
    let uow_factory = CarReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
