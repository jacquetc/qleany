...
use super::{
    ...
    units_of_work::{CustomerReadUoWFactory, CustomerWriteUoWFactory},
};
...
pub fn create_orphan(
    ...
    let uow_factory = CustomerWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::Customer = entity.into();
...
pub fn create_orphan_multi(
    ...
    let uow_factory = CustomerWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::Customer> =
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<CustomerDto>> {
    let uow_factory = CustomerReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<CustomerDto>> {
    let uow_factory = CustomerReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
