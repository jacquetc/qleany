...
/// Use case to remove multiple customer entities. The undo and redo operations are deactivated
...
impl RemoveCustomerUseCase {
    ...
    pub fn execute(&mut self, id: &EntityId) -> Result<()> {
        ...
        if uow.get_customer(&id)?.is_none() {
            return Err(anyhow::anyhow!("Customer with id {} does not exist", id));
        }
        uow.delete_customer(id)?;
        uow.commit()?;
...
impl UndoRedoCommand for RemoveCustomerUseCase {
