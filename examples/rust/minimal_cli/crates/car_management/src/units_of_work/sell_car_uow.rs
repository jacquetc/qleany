...
use crate::use_cases::sell_car_uc::{SellCarUnitOfWorkFactoryTrait, SellCarUnitOfWorkTrait};
...
// Unit of work for SellCar

pub struct SellCarUnitOfWork {
    context: DbContext,
    transaction: Option<Transaction>,
    event_hub: Arc<EventHub>,
}

impl SellCarUnitOfWork {
...
impl CommandUnitOfWork for SellCarUnitOfWork {
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/sell_car_uc.rs
//
...
impl SellCarUnitOfWorkTrait for SellCarUnitOfWork {}

pub struct SellCarUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl SellCarUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        SellCarUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl SellCarUnitOfWorkFactoryTrait for SellCarUnitOfWorkFactory {
    fn create(&self) -> Box<dyn SellCarUnitOfWorkTrait> {
        Box::new(SellCarUnitOfWork::new(&self.context, &self.event_hub))
    }
}
