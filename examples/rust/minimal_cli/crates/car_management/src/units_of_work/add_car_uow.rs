...
use crate::use_cases::add_car_uc::{AddCarUnitOfWorkFactoryTrait, AddCarUnitOfWorkTrait};
...
// Unit of work for AddCar

pub struct AddCarUnitOfWork {
    context: DbContext,
    transaction: Option<Transaction>,
    event_hub: Arc<EventHub>,
}

impl AddCarUnitOfWork {
...
impl CommandUnitOfWork for AddCarUnitOfWork {
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/add_car_uc.rs
//
...
impl AddCarUnitOfWorkTrait for AddCarUnitOfWork {}

pub struct AddCarUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl AddCarUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        AddCarUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl AddCarUnitOfWorkFactoryTrait for AddCarUnitOfWorkFactory {
    fn create(&self) -> Box<dyn AddCarUnitOfWorkTrait> {
        Box::new(AddCarUnitOfWork::new(&self.context, &self.event_hub))
    }
}
