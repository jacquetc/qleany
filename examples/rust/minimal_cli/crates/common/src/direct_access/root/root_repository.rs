...
impl<'a> RootRepository<'a> {
    ...
    pub fn delete(&mut self, event_hub: &Arc<EventHub>, id: &EntityId) -> Result<(), Error> {
        ...
        let cars = entity.cars.clone();

        let customers = entity.customers.clone();

        // delete all strong relationships, initiating a cascade delete

        repository_factory::write::create_car_repository(self.transaction)
            .delete_multi(event_hub, &cars)?;

        repository_factory::write::create_customer_repository(self.transaction)
            .delete_multi(event_hub, &customers)?;
    ...
    pub fn delete_multi(
        ...
        let mut cars_ids: Vec<EntityId> = entities
            .iter()
            .flat_map(|entity| entity.as_ref().map(|entity| entity.cars.clone()))
            .flatten()
            .collect();
        // remove duplicates
        cars_ids.sort();
        ...
        let mut customers_ids: Vec<EntityId> = entities
            .iter()
            .flat_map(|entity| entity.as_ref().map(|entity| entity.customers.clone()))
            .flatten()
            .collect();
        // remove duplicates
        customers_ids.sort();
        ...
        repository_factory::write::create_car_repository(self.transaction)
            .delete_multi(event_hub, &cars_ids)?;

        repository_factory::write::create_customer_repository(self.transaction)
            .delete_multi(event_hub, &customers_ids)?;
