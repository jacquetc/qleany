...
struct SIMPLE_EXAMPLE_CONTROLLER_EXPORT Scope
...
inline bool operator==(const Simple::Controller::UndoRedo::Scope &s1, const Simple::Controller::UndoRedo::Scope &s2)
{
    return s1.scope() == s2.scope();
}

// Define the qHash function for Scope
inline uint qHash(const Simple::Controller::UndoRedo::Scope &scope, uint seed = 0)
{
    return scope.scope();
}

// Scopes class to manage multiple scopes for undo-redo commands
class SIMPLE_EXAMPLE_CONTROLLER_EXPORT Scopes
{
    Q_GADGET
...
Q_DECLARE_METATYPE(Simple::Controller::UndoRedo::Scope)