                                                         ...
                                                         const UpdateClientCommand &request)
{
    ...
    if (m_undoState.isEmpty())
    {
        ...
        m_undoState = Result<ClientDTO>(
            Qleany::Tools::AutoMapper::AutoMapper::map<FrontEnds::Entities::Client, ClientDTO>(currentResult.value()));
    }
    auto updateDto = Qleany::Tools::AutoMapper::AutoMapper::map<ClientDTO, UpdateClientDTO>(m_undoState.value());
    updateDto << request.req;

    // map
    auto client = Qleany::Tools::AutoMapper::AutoMapper::map<UpdateClientDTO, FrontEnds::Entities::Client>(updateDto);
...
Result<ClientDTO> UpdateClientCommandHandler::restoreImpl()
{
    ...
    // map
    auto client =
        Qleany::Tools::AutoMapper::AutoMapper::map<ClientDTO, FrontEnds::Entities::Client>(m_undoState.value());
