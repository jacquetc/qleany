...
public:
    ...
    static void registerMapping(bool isDefault = false, bool reverseConversion = false)
    {
        ...
        getSiblingFunctions[MetaTypePair(QMetaType::fromType<QList<SourceType>>(), QMetaType::fromType<QList<DestinationType>>())] = []() {
            return QVariant::fromValue(DestinationType());
        };
        if (isDefault) {
        ...
        writerForListHash[MetaTypePair(QMetaType::fromType<QList<SourceType>>(), QMetaType::fromType<DestinationType>())] =
        ...
        if (reverseConversion) {
            getSiblingFunctions[MetaTypePair(QMetaType::fromType<DestinationType>(), QMetaType::fromType<SourceType>())] = []() {
            ...
            getSiblingFunctions[MetaTypePair(QMetaType::fromType<QList<DestinationType>>(), QMetaType::fromType<SourceType>())] = []() {
                return QVariant::fromValue(SourceType());
            };
            if (isDefault) {
            ...
            writerForListHash[MetaTypePair(QMetaType::fromType<QList<DestinationType>>(), QMetaType::fromType<SourceType>())] =
                [](const QMetaProperty &destinationProperty, void *gadgetPointer, const QList<QVariant> &sourceList) {
...
private:
    ...
    static QVariant implDefaultMap(const QVariant &source)
    {
            ...
            if (sourceProperty.isReadable()) {
                ...
                if (destinationPropertyIndex >= 0) {
                    ...
                    if (destinationProperty.isWritable() && QMetaType::canConvert(value.metaType(), destinationProperty.metaType())) {
                        ...
                        if (!success) {
                            qWarning() << "Failed to write value" << value << "to destination property" << destinationProperty.name();
                        }
                    }

                    else {
                        qCritical(
                            "AutoMapper error: Property types do not match. Check the types of the source and "
    ...
    template<class SourceType, class DestinationType>
    static QVariant implMap(const SourceType &source)
    {
        // list all metatypes in getSiblingFunctions
        //        for (auto it = getSiblingFunctions.begin(); it != getSiblingFunctions.end(); ++it)
        //        {
                    ...
                    if (destinationProperty.isWritable() && QMetaType::canConvert(value.metaType(), destinationProperty.metaType())) {
                        ...
                        if (getDefaultSiblingFunction != getDefaultSiblingFunctions.end()) {
                            ...
                            if (QString::fromLatin1(value.metaType().name()).startsWith("QList<"_L1)) {
                                ...
                                auto writeForListIt = writerForListHash.find(MetaTypePair(value.metaType(), getDefaultSiblingFunction.value()().metaType()));

                                if (writeForListIt != writerForListHash.end() /*&&
                                    source.metaData().getSet(sourceProperty.name())*/)
                                {
                                    bool success = writeForListIt.value()(destinationProperty, destinationPointer, sourceList);

                                    if (!success) {
                                        qWarning() << "Failed to write value" << destinationValue << "to destination property" << destinationProperty.name();
                                    }
                                }
                            } else {
                                // It's a single QVariant with custom type.

                                auto writeIt = writerHash.find(MetaTypePair(value.metaType(), getDefaultSiblingFunction.value()().metaType()));

                                if (writeIt != writerHash.end() && source.metaData().getSet(QString::fromLatin1(sourceProperty.name()))) {
...
private:
    ...
    static QHash<MetaTypePair, std::function<bool(const QMetaProperty &destinationProperty, void *gadgetPointer, const QVariant &sourceValue)>> writerHash;
    static QHash<MetaTypePair, std::function<bool(const QMetaProperty &destinationProperty, void *gadgetPointer, const QList<QVariant> &sourceList)>>
        writerForListHash;
