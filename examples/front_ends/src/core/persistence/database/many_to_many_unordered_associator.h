...
{
public:
    ManyToManyUnorderedAssociator(QSharedPointer<InterfaceDatabaseContext> context, const FrontEnds::Entities::RelationshipInfo &relationship)
...
template<class RightEntity>
QString ManyToManyUnorderedAssociator<RightEntity>::getTableCreationSql() const
{
    return "CREATE TABLE IF NOT EXISTS %1"
           ...
           "UNIQUE (%8, %9) ON CONFLICT ROLLBACK);"_L1.arg(m_junctionTableName,
...
template<class RightEntity>
Result<QList<RightEntity>> ManyToManyUnorderedAssociator<RightEntity>::updateRightEntities(int leftEntityId, const QList<RightEntity> &rightEntities)
{
    ...
    for (int rightEntityId : rightEntityIds) {
        if (!rightEntities.contains(rightEntityId)) {
            ...
            QString deleteQueryStr = "DELETE FROM "_L1 + m_junctionTableName + " WHERE "_L1 + m_junctionTableLeftEntityForeignKeyName
                + " = :leftEntityId AND "_L1 + m_junctionTableRightEntityForeignKeyName + " = :rightEntityId"_L1;
            ...
            if (!deleteQuery.exec()) {
                return Result<QList<RightEntity>>(QLN_ERROR_3(Q_FUNC_INFO, Error::Critical, "sql_error", deleteQuery.lastError().text(), deleteQueryStr));
            }
        }
    }
    // add the new ones
    ...
    for (int rightEntityId : rightEntityIds) {
        ...
        if (!insertQuery.exec()) {
            return Result<QList<RightEntity>>(QLN_ERROR_3(Q_FUNC_INFO, Error::Critical, "sql_error", insertQuery.lastError().text(), insertQueryStr));
        }
    }
...
template<class RightEntity>
Result<QList<RightEntity>> ManyToManyUnorderedAssociator<RightEntity>::getRightEntitiesFromTheirIds(QList<int> rightEntityIds) const
{
...
template<class RightEntity>
QStringList ManyToManyUnorderedAssociator<RightEntity>::getTablePropertyColumns(const FrontEnds::Entities::EntitySchema &entitySchema) const
{
    QStringList columns;
