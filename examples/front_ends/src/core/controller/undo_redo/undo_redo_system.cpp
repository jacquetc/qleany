...
UndoRedoSystem::UndoRedoSystem(QObject *parent, Scopes scopes)
    ...
    , m_scopes(scopes)
{
    qRegisterMetaType<FrontEnds::Controller::UndoRedo::Scope>();
...
{
    return m_activeStack->currentIndex() < m_activeStack->queue().size() - 1 && !m_activeStack->queue()[m_activeStack->currentIndex() + 1]->isRunning();
}
...
void UndoRedoSystem::undo()
{
    if (canUndo()) {
        ...
        // signal
        connect(command.data(), &UndoRedoCommand::finished, this, &UndoRedoSystem::onCommandUndoFinished, Qt::SingleShotConnection);
...
void UndoRedoSystem::redo()
{
    if (canRedo()) {
        ...
        //  single shot connection to avoid unwanted
        // signal
        connect(command.data(), &UndoRedoCommand::finished, this, &UndoRedoSystem::onCommandRedoFinished, Qt::SingleShotConnection);
...
void UndoRedoSystem::setCurrentIndex(int index)
{
    ...
    if (index < m_activeStack->currentIndex()) {
        ...
        while (movingIndex > index) {
            ...
            connect(command.data(), &UndoRedoCommand::finished, this, &UndoRedoSystem::executeNextCommandUndo, Qt::SingleShotConnection);
        ...
        while (movingIndex < index) {
            movingIndex++;
            auto command = m_activeStack->queue()[movingIndex];
            connect(command.data(), &UndoRedoCommand::finished, this, &UndoRedoSystem::executeNextCommandRedo, Qt::SingleShotConnection);
            m_setIndexCommandQueue.enqueue(command);
        }
...
void UndoRedoSystem::executeNextCommandDo(const ScopeFlag &scopeFlag, const QUuid &stackId)
{
    ...
    if (allowedToRun) {
        ...
        connect(command.data(), &UndoRedoCommand::finished, this, &UndoRedoSystem::onCommandDoFinished, Qt::SingleShotConnection);
