...
struct FRONT_ENDS_EXAMPLE_CONTROLLER_EXPORT Scope
...
inline bool operator==(const FrontEnds::Controller::UndoRedo::Scope &s1, const FrontEnds::Controller::UndoRedo::Scope &s2)
{
    return s1.scope() == s2.scope();
}

// Define the qHash function for Scope
inline uint qHash(const FrontEnds::Controller::UndoRedo::Scope &scope, uint seed = 0)
{
    return scope.scope();
}

// Scopes class to manage multiple scopes for undo-redo commands
class FRONT_ENDS_EXAMPLE_CONTROLLER_EXPORT Scopes
{
    Q_GADGET
...
Q_DECLARE_METATYPE(FrontEnds::Controller::UndoRedo::Scope)