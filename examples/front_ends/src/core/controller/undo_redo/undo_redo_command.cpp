    ...
    , m_status(Status::Waiting)
{
    ...
    connect(m_watcher, &QFutureWatcher<Result<void>>::progressRangeChanged, this, &UndoRedoCommand::progressRangeChanged);
    connect(m_watcher, &QFutureWatcher<Result<void>>::progressTextChanged, this, &UndoRedoCommand::progressTextChanged);
    connect(m_watcher, &QFutureWatcher<Result<void>>::progressValueChanged, this, &UndoRedoCommand::progressValueChanged);
}
...
void UndoRedoCommand::asyncRedo()
{
    ...
    m_watcher->setFuture(QtConcurrent::run(m_redoFunction).onFailed([](const std::exception &e) {
        return Result<void>(QLN_ERROR_2(Q_FUNC_INFO,
                                        Error::Critical,
                                        "redo-error",
