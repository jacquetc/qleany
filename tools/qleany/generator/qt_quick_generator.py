...
def _generate_cmakelists_file(
    ...
    template_env = Environment(loader=FileSystemLoader("templates/UIs/qt_quick"))
    # Load the template
    ...
    output = cmakelists_template.render(
        ...
        application_spinal_name=stringcase.spinalcase(
            cmakelists_dict["application_name"]
        ),
        application_pascal_name=stringcase.pascalcase(
            cmakelists_dict["application_name"]
        ),
    )
...
def _generate_qml_file(
    ...
    file = os.path.join(qml_dict["folder_path"], "content/App.qml")
    # Create the jinja2 environment
    template_env = Environment(
        loader=FileSystemLoader("templates/UIs/qt_quick/content")
    )
    # Load the template
    qml_template = template_env.get_template("App.qml.jinja2")
...
def _generate_other_files(
    root_path: str, folder_path: str, files_to_be_generated: dict[str, bool]
):
    # copy the qtquickcontrols2.conf file
    source = os.path.join("templates/UIs/qt_quick", "qtquickcontrols2.conf")
    destination = os.path.join(folder_path, "qtquickcontrols2.conf")

    # generate the qtquickcontrols2.conf file if in the files_to_be_generated dict the value is True
    ...
    # copy the main.qml file
    source = os.path.join("templates/UIs/qt_quick", "main.qml")
    destination = os.path.join(folder_path, "main.qml")

    # generate the main.qml file if in the files_to_be_generated dict the value is True
    if files_to_be_generated.get(destination, False):
    ...
    # copy the content's CMakeLists.txt file
    source = os.path.join("templates/UIs/qt_quick/content", "CMakeLists.txt")
    destination = os.path.join(folder_path, "content/CMakeLists.txt")

    # generate the content's CMakeLists.txt file if in the files_to_be_generated dict the value is True
    if files_to_be_generated.get(destination, False):
...
def generate_qt_quick_files(
    ...
    _generate_other_files(root_path, folder_path, files_to_be_generated)
...
def get_files_to_be_generated(
    ...
    files.append(os.path.join(folder_path, "main.qml"))
    files.append(os.path.join(folder_path, "content/App.qml"))
    files.append(os.path.join(folder_path, "content/CMakeLists.txt"))
    files.append(os.path.join(folder_path, "qtquickcontrols2.conf"))
    ...
    if len(sys.argv) > 1:
        ...
        if manifest_arg.endswith(".yaml") or manifest_arg.endswith(".yml"):
            ...
            if len(sys.argv) > 2 and sys.argv[2] == "--preview":
                preview_qt_quick_files(root_path, manifest_file)
            else:
                generate_qt_quick_files(root_path, manifest_file)
        else:
