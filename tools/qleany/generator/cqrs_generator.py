...
def generate_query(
    root_path: str, query: dict, files_to_be_generated: dict[str, bool] = None
):
...
def generate_command(
    root_path: str, command: dict, files_to_be_generated: dict[str, bool] = None
):
    template_env = Environment(loader=FileSystemLoader("templates/CQRS"))
...
def generate_validator(
    root_path: str, validator: dict, files_to_be_generated: dict[str, bool] = None
):
    template_env = Environment(loader=FileSystemLoader("templates/CQRS"))
...
def generate_feature_cmake(
    ...
    cmake_file_path = os.path.join(root_path, cmake_file_path)
    ...
    for file_path in files:
        relative_generated_file = os.path.relpath(
            os.path.join(root_path, file_path), os.path.dirname(cmake_file_path)
        )
...
def generate_cmake(
    root_path: str, cqrs_dict: dict, files_to_be_generated: dict[str, bool] = None
):
    ...
    for feature_name, feature in cqrs_dict["features"].items():
        ...
        for query_name, query in feature["queries"].items():
            generated_files.append(
                generate_query(root_path, query, files_to_be_generated)
            )
        for command_name, command in feature["commands"].items():
            generated_files.append(
                generate_command(root_path, command, files_to_be_generated)
            )
        for validator_name, validator in feature["validators"].items():
            generated_files.append(
                generate_validator(root_path, validator, files_to_be_generated)
            )
        ...
        generate_feature_cmake(
            root_path, application_name, feature, files_to_be_generated
        )

    # generate CMakeLists.txt
    generate_cmake(root_path, cqrs_dict, files_to_be_generated)
...
def preview_cqrs_files(
    ...
    manifest["CQRS"]["common_cmake_folder_path"] = manifest["CQRS"][
    ...
    if len(sys.argv) > 1:
        ...
        if manifest_arg.endswith(".yaml") or manifest_arg.endswith(".yml"):
            ...
            if len(sys.argv) > 2 and sys.argv[2] == "--preview":
                preview_cqrs_files(root_path, manifest_file)
            else:
                # generate the files
                generate_cqrs_files(root_path, manifest_file)
        else:
