...
def _generate_generic_repository_and_repository_provider(
    root_path: str,
    repository_path: str,
    ...
    template_env = Environment(
        loader=FileSystemLoader("templates/persistence/repository")
    )

    files = ["generic_repository.h", "repository_provider.h", "repository_provider.cpp"]
    ...
    for file in files:
        ...
        if not files_to_be_generated.get(os.path.join(repository_path, file), False):
            continue

        output_file = os.path.join(root_path, repository_path, file)
...
def _generate_database_files(
    ...
    template_env = Environment(
        loader=FileSystemLoader("templates/persistence/database")
    )

    files = [
        "database_context.h",
        "database_context.cpp",
        "database_table_group.h",
        "many_to_many_unordered_associator.h",
        "one_to_many_unordered_associator.h",
    ...
    for file in files:
        ...
        # generate the real file if in the files_to_be_generated dict the value is True
        if not files_to_be_generated.get(os.path.join(database_path, file), False):
            continue

        output_file = os.path.join(root_path, database_path, file)
...
def _generate_database_interfaces(
    root_path: str,
    contracts_folder_path: str,
    application_cpp_domain_name: str,
    export: str,
    export_header_file: str,
    files_to_be_generated: dict[str, bool],
    ...
    for file in files:
        ...
        if not files_to_be_generated.get(
            os.path.join(contracts_folder_path, "database", file), False
        ):
            continue

        output_file = os.path.join(root_path, contracts_folder_path, "database", file)
...
def _generate_repository_interfaces(
    root_path: str,
    contracts_folder_path: str,
    application_cpp_domain_name: str,
    export: str,
    ...
    template_env = Environment(
        loader=FileSystemLoader("templates/persistence/contracts/repository")
    )

    files = [
        "interface_generic_repository.h",
        "interface_repository_provider.h",
    ...
    for file in files:
        ...
        # generate the real file if in the files_to_be_generated dict the value is True
        if not files_to_be_generated.get(
            os.path.join(contracts_folder_path, "repository", file), False
        ):
            continue

        output_file = os.path.join(root_path, contracts_folder_path, "repository", file)
...
def _generate_cmakelists(
    ...
    template_env = Environment(loader=FileSystemLoader("templates/persistence"))
...
def _generate_contracts_cmakelists(
    ...
    template_env = Environment(
        loader=FileSystemLoader("templates/persistence/contracts")
    )
    cmakelists_template = template_env.get_template("cmakelists_template.jinja2")
...
def generate_persistence_files(
    root_path: str,
    manifest_file,
    ...
    persistence_data = manifest_data.get("persistence", {})
    repositories_list = persistence_data.get("list", [])
    repository_path = persistence_data.get("repository_folder_path", ".")
    base_path = persistence_data.get("base_folder_path", ".")
    ...
    interface_repository_path = os.path.join(contracts_folder_path, "repository")
    interface_database_path = os.path.join(contracts_folder_path, "database")
    ...
    export_header_file = (
        f"{stringcase.snakecase(application_name)}_persistence_export.h"
    )
    contracts_export = (
        f"{stringcase.snakecase(application_name).upper()}_CONTRACTS_EXPORT"
    )
    contracts_export_header_file = (
        f"{stringcase.snakecase(application_name)}_contracts_export.h"
    )
    ...
    for repository in repositories_list:
        ...
        for key, value in foreign_entities.items():
            ...
            if new_constructor_argument not in foreign_repository_constructor_arguments:
        ...
        # remove duplicates :
        foreign_entities_private_member_list = list(
            dict.fromkeys(foreign_entities_private_member_list)
        )
        ...
        output_file = os.path.join(
            interface_repository_path, f"interface_{snake_name}_repository.h"
        )
    ...
    if files_to_be_generated.get(relative_cmake_output_file, False):
        ...
        absolute_repository_path = os.path.join(root_path, repository_path)

        all_repository_files.append(
            os.path.join(absolute_repository_path, "generic_repository.h")
        )
        all_repository_files.append(
            os.path.join(absolute_repository_path, "repository_provider.h")
        )
        all_repository_files.append(
            os.path.join(absolute_repository_path, "repository_provider.cpp")
        )
    ...
    if files_to_be_generated.get(
        ...
        absolute_interface_repository_path = os.path.join(
            root_path, interface_repository_path
        )

        all_interface_files.extend(
            [
                os.path.join(
                    absolute_interface_repository_path, "interface_generic_repository.h"
                ),
                os.path.join(
                    absolute_interface_repository_path,
                    "interface_repository_provider.h",
                ),
                os.path.join(
                    absolute_interface_repository_path, "interface_repository.h"
                ),
                os.path.join(absolute_interface_repository_path, "signal_holder.h"),
                os.path.join(absolute_interface_repository_path, "signal_holder.cpp"),
            ]
        )
        absolute_interface_database_path = os.path.join(
            root_path, interface_database_path
        )

        all_interface_files.append(
            os.path.join(
                absolute_interface_database_path, "interface_database_context.h"
            )
        )
        all_interface_files.append(
            os.path.join(
    ...
    # write database interfaces

    _generate_database_interfaces(
        root_path,
        contracts_folder_path,
    ...
    _generate_repository_interfaces(
        root_path,
        contracts_folder_path,
        application_cpp_domain_name,
        contracts_export,
    ...
    _generate_generic_repository_and_repository_provider(
        root_path,
        repository_path,
        application_cpp_domain_name,
        export,
...
def get_files_to_be_generated(
    ...
    base_folder_path = manifest["persistence"]["base_folder_path"]
    repository_folder_path = manifest["persistence"]["repository_folder_path"]
    database_older_path = os.path.join(base_folder_path, "database")
    contracts_folder_path = manifest["contracts"]["folder_path"]
    interface_repository_folder_path = os.path.join(contracts_folder_path, "repository")
    interface_database_folder_path = os.path.join(contracts_folder_path, "database")
    global_data = manifest.get("global", {})

    # Get the list of files to be generated
    files = []
    for entity in manifest["persistence"]["list"]:
        ...
        files.append(
            os.path.join(
                interface_repository_folder_path,
    ...
    for database_file in database_files:
        files.append(os.path.join(database_older_path, database_file))

    files.append(os.path.join(repository_folder_path, "generic_repository.h"))
    files.append(os.path.join(repository_folder_path, "repository_provider.h"))
    files.append(os.path.join(repository_folder_path, "repository_provider.cpp"))

    interface_repository_files = [
        "interface_generic_repository.h",
        "interface_repository_provider.h",
        "interface_repository.h",
        "signal_holder.h",
    ...
    for interface_repository_file in interface_repository_files:
        files.append(
            os.path.join(interface_repository_folder_path, interface_repository_file)
        )

    files.append(
        os.path.join(interface_database_folder_path, "interface_database_context.h")
    )
    files.append(
...
def preview_persistence_files(
    ...
    manifest["persistence"]["base_folder_path"] = manifest["persistence"][
        "base_folder_path"
    ].replace("..", "")
    manifest["persistence"]["repository_folder_path"] = manifest["persistence"][
        "repository_folder_path"
    ].replace("..", "")
    ...
    if len(sys.argv) > 1:
        ...
        if manifest_arg.endswith(".yaml") or manifest_arg.endswith(".yml"):
            ...
            if len(sys.argv) > 2 and sys.argv[2] == "--preview":
                preview_persistence_files(root_path, manifest_file)
            else:
                generate_persistence_files(root_path, manifest_file)
        else:
