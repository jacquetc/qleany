...
fn test_snapshot_serialization_roundtrip() {
        ...
        children: vec![EntityTreeSnapshot {
            table_data: TableLevelSnapshot {
                entity_rows: TableSnapshot {
                    ...
                    rows: vec![(10, vec![20, 21]), (11, vec![22, 23]), (12, vec![24, 25])],
                },
    ...
    let deserialized: EntityTreeSnapshot =
        postcard::from_bytes(&serialized).expect("deserialize failed");

    assert_eq!(
        deserialized.table_data.entity_rows.table_name,
        "test_entity"
    );
    ...
    assert_eq!(
        deserialized.children[0].table_data.entity_rows.rows.len(),
        3
    );
}
...
fn test_snapshot_deeply_nested() {
    ...
    // Verify round-trip
    let serialized = postcard::to_allocvec(&root).expect("serialize failed");
    let deserialized: EntityTreeSnapshot =
        postcard::from_bytes(&serialized).expect("deserialize failed");
