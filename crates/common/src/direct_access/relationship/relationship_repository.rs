...
pub struct RelationshipRepository<'a> {
    table: Box<dyn RelationshipTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> RelationshipRepository<'a> {
    pub fn new(table: Box<dyn RelationshipTable + 'a>, transaction: &'a Transaction) -> Self {
        RelationshipRepository { table, transaction }
    }
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&EntityRelationshipField::Relationships, &[*id])?
                .into_iter()
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_entity_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&EntityRelationshipField::Relationships, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn restore(
        ...
        let relationship_ids: Vec<_> = store
            .relationships
            .read()
            .unwrap()
            .keys()
...
pub struct RelationshipRepositoryRO<'a> {
    table: Box<dyn RelationshipTableRO + 'a>,
}
impl<'a> RelationshipRepositoryRO<'a> {
    pub fn new(table: Box<dyn RelationshipTableRO + 'a>) -> Self {
        RelationshipRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Relationship>, RepositoryError> {
        self.table.get(id)
    }
