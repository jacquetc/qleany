...
pub struct DtoRepository<'a> {
    table: Box<dyn DtoTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> DtoRepository<'a> {
    pub fn new(table: Box<dyn DtoTable + 'a>, transaction: &'a Transaction) -> Self {
        DtoRepository { table, transaction }
    }
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&UseCaseRelationshipField::DtoIn, &[*id])?
                .into_iter()
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo =
                repository_factory::write::create_use_case_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&UseCaseRelationshipField::DtoIn, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn restore(
        ...
        let dto_ids: Vec<_> = store.dtos.read().unwrap().keys().copied().collect();
        emit(
            DirectAccessEntity::Dto(EntityEvent::Created),
            dto_ids.clone(),
        );
        emit(DirectAccessEntity::Dto(EntityEvent::Updated), dto_ids);

        // Emit Created events for strong children

        {
            let child_ids: Vec<_> = store.dto_fields.read().unwrap().keys().copied().collect();
            emit(
...
pub struct DtoRepositoryRO<'a> {
    table: Box<dyn DtoTableRO + 'a>,
}
impl<'a> DtoRepositoryRO<'a> {
    pub fn new(table: Box<dyn DtoTableRO + 'a>) -> Self {
        DtoRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Dto>, RepositoryError> {
        self.table.get(id)
    }
