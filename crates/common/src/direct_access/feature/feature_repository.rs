...
pub struct FeatureRepository<'a> {
    table: Box<dyn FeatureTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> FeatureRepository<'a> {
    pub fn new(table: Box<dyn FeatureTable + 'a>, transaction: &'a Transaction) -> Self {
        FeatureRepository { table, transaction }
    }
    ...
    pub fn remove(
        ...
        repository_factory::write::create_use_case_repository(self.transaction)?
            .remove_multi(event_buffer, &use_cases)?;
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&WorkspaceRelationshipField::Features, &[*id])?
                .into_iter()
    ...
    pub fn remove_multi(
        ...
        repository_factory::write::create_use_case_repository(self.transaction)?
            .remove_multi(event_buffer, &use_cases_ids)?;
        // Before removal, find which owner(s) reference these entities
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo =
                repository_factory::write::create_workspace_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&WorkspaceRelationshipField::Features, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn restore(
        ...
        let feature_ids: Vec<_> = store.features.read().unwrap().keys().copied().collect();
        emit(
        ...
        {
            let child_ids: Vec<_> = store.use_cases.read().unwrap().keys().copied().collect();
            emit(
                DirectAccessEntity::UseCase(EntityEvent::Created),
                child_ids.clone(),
            );
            emit(DirectAccessEntity::UseCase(EntityEvent::Updated), child_ids);
        }
...
pub struct FeatureRepositoryRO<'a> {
    table: Box<dyn FeatureTableRO + 'a>,
}
impl<'a> FeatureRepositoryRO<'a> {
    pub fn new(table: Box<dyn FeatureTableRO + 'a>) -> Self {
        FeatureRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Feature>, RepositoryError> {
        self.table.get(id)
    }
