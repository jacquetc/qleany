...
pub struct FieldRepository<'a> {
    table: Box<dyn FieldTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> FieldRepository<'a> {
    pub fn new(table: Box<dyn FieldTable + 'a>, transaction: &'a Transaction) -> Self {
        FieldRepository { table, transaction }
    }
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Field>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Field>>, RepositoryError> {
        self.table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Field>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_entity_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&EntityRelationshipField::Fields, &[*id])?
                .into_iter()
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_entity_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&EntityRelationshipField::Fields, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn restore(
        ...
        let field_ids: Vec<_> = store.fields.read().unwrap().keys().copied().collect();
        emit(
            DirectAccessEntity::Field(EntityEvent::Created),
            field_ids.clone(),
        );
        emit(DirectAccessEntity::Field(EntityEvent::Updated), field_ids);
...
pub struct FieldRepositoryRO<'a> {
    table: Box<dyn FieldTableRO + 'a>,
}
impl<'a> FieldRepositoryRO<'a> {
    pub fn new(table: Box<dyn FieldTableRO + 'a>) -> Self {
        FieldRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Field>, RepositoryError> {
        self.table.get(id)
    }
