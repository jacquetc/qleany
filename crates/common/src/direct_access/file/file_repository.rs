...
pub struct FileRepository<'a> {
    table: Box<dyn FileTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> FileRepository<'a> {
    pub fn new(table: Box<dyn FileTable + 'a>, transaction: &'a Transaction) -> Self {
        FileRepository { table, transaction }
    }
    ...
    pub fn restore(
        ...
        let file_ids: Vec<_> = store.files.read().unwrap().keys().copied().collect();
        emit(
            DirectAccessEntity::File(EntityEvent::Created),
            file_ids.clone(),
        );
        emit(DirectAccessEntity::File(EntityEvent::Updated), file_ids);
...
pub struct FileRepositoryRO<'a> {
    table: Box<dyn FileTableRO + 'a>,
}
impl<'a> FileRepositoryRO<'a> {
    pub fn new(table: Box<dyn FileTableRO + 'a>) -> Self {
        FileRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<File>, RepositoryError> {
        self.table.get(id)
    }
