...
impl<'a> SystemRepository<'a> {
    ...
    pub fn restore(
        ...
        let system_ids: Vec<_> = store.systems.read().unwrap().keys().copied().collect();
        emit(
            DirectAccessEntity::System(EntityEvent::Created),
            system_ids.clone(),
        );
        emit(DirectAccessEntity::System(EntityEvent::Updated), system_ids);

        // Emit Created events for strong children

        {
            let child_ids: Vec<_> = store.files.read().unwrap().keys().copied().collect();
            emit(
                DirectAccessEntity::File(EntityEvent::Created),
                child_ids.clone(),
            );
            emit(DirectAccessEntity::File(EntityEvent::Updated), child_ids);
        }
