...
pub trait DtoFieldTable {
    ...
    fn update_with_relationships(&mut self, entity: &DtoField)
...
pub struct DtoFieldRepository<'a> {
    table: Box<dyn DtoFieldTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> DtoFieldRepository<'a> {
    pub fn new(table: Box<dyn DtoFieldTable + 'a>, transaction: &'a Transaction) -> Self {
        DtoFieldRepository { table, transaction }
    }
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_dto_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&DtoRelationshipField::Fields, &[*id])?
                .into_iter()
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_dto_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&DtoRelationshipField::Fields, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn restore(
        ...
        let dto_field_ids: Vec<_> = store.dto_fields.read().unwrap().keys().copied().collect();
        emit(
...
pub struct DtoFieldRepositoryRO<'a> {
    table: Box<dyn DtoFieldTableRO + 'a>,
}
impl<'a> DtoFieldRepositoryRO<'a> {
    pub fn new(table: Box<dyn DtoFieldTableRO + 'a>) -> Self {
        DtoFieldRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<DtoField>, RepositoryError> {
        self.table.get(id)
    }
