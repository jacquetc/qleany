...
pub trait GlobalTable {
    ...
    fn update_with_relationships(&mut self, entity: &Global) -> Result<Global, RepositoryError>;
...
pub struct GlobalRepository<'a> {
    table: Box<dyn GlobalTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> GlobalRepository<'a> {
    pub fn new(table: Box<dyn GlobalTable + 'a>, transaction: &'a Transaction) -> Self {
        GlobalRepository { table, transaction }
    }
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&WorkspaceRelationshipField::Global, &[*id])?
                .into_iter()
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo =
                repository_factory::write::create_workspace_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&WorkspaceRelationshipField::Global, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn restore(
        ...
        let global_ids: Vec<_> = store.globals.read().unwrap().keys().copied().collect();
        emit(
...
pub struct GlobalRepositoryRO<'a> {
    table: Box<dyn GlobalTableRO + 'a>,
}
impl<'a> GlobalRepositoryRO<'a> {
    pub fn new(table: Box<dyn GlobalTableRO + 'a>) -> Self {
        GlobalRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Global>, RepositoryError> {
        self.table.get(id)
    }
