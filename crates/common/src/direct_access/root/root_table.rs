...
impl<'a> RootHashMapTable<'a> {
    ...
    fn resolve_junction(
        ...
        match field {
            ...
            RootRelationshipField::Workspace => &self.store.jn_workspace_from_root_workspace,
        }
    }

    fn hydrate(&self, entity: &mut Root) {
        entity.workspace = junction_get(&self.store.jn_workspace_from_root_workspace, &entity.id)
            .into_iter()
            .next();
        entity.system = junction_get(&self.store.jn_system_from_root_system, &entity.id)
            .into_iter()
            .next();
    }
}
...
impl<'a> RootTable for RootHashMapTable<'a> {
    ...
    fn create_multi(&mut self, entities: &[Root]) -> Result<Vec<Root>, RepositoryError> {
        ...
        for entity in entities {
            ...
            junction_set(
                &self.store.jn_workspace_from_root_workspace,
                new_entity.id,
    ...
    fn remove_multi(&mut self, ids: &[EntityId]) -> Result<(), RepositoryError> {
        ...
        for id in ids {
            ...
            junction_remove(&self.store.jn_workspace_from_root_workspace, id);
