...
use direct_access::{DtoRelationshipDto, FeatureRelationshipDto, UseCaseRelationshipDto};
...
fn subscribe_feature_updated_event(
    event_hub_client: &EventHubClient,
    app: &App,
    app_context: &Arc<AppContext>,
) {
...
fn subscribe_use_case_updated_event(
    event_hub_client: &EventHubClient,
    app: &App,
    app_context: &Arc<AppContext>,
) {
            ...
            if let Some(app) = app_weak.upgrade() {
                if new_feature_name != "" {
                    let current_feature_id =
                        app.global::<FeaturesTabState>().get_selected_feature_id();
                    let feature_res = feature_commands::get_feature(
                        &ctx,
