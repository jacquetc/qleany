...
pub fn fill_dto_in_field_form(app: &App, dto_field: &direct_access::DtoFieldDto) {
    ...
    state.set_selected_dto_in_field_enum_values(dto_field.enum_values.join("\n").into());
}
            ...
            if let Some(app) = app_weak.upgrade() {
                ...
                if enabled {
                    ...
                    let default_name = match use_case_commands::get_use_case(
                        ...
                        Ok(Some(uc)) => {
                            format!("{}Dto", heck::AsPascalCase(&uc.name))
                        }
                        _ => "NewDtoIn".to_string(),
                    };
        ...
        .on_dto_in_field_optional_changed({
            ...
            move |optional| {
                if let Some(app) = app_weak.upgrade() {
                    ...
                    if optional {
                        app.global::<FeaturesTabState>()
                            .set_selected_dto_in_field_is_list(false);
                    }
                }
            }
        });
        ...
        .on_dto_in_field_is_list_changed({
            ...
            move |is_list| {
                if let Some(app) = app_weak.upgrade() {
                    ...
                    if is_list {
                        app.global::<FeaturesTabState>()
                            .set_selected_dto_in_field_optional(false);
                    }
                }
            }
        });
