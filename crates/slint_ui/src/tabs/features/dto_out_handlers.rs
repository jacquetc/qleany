...
pub fn fill_dto_out_field_form(app: &App, dto_field: &direct_access::DtoFieldDto) {
    ...
    state.set_selected_dto_out_field_optional(dto_field.optional);
...
pub fn clear_dto_out_field_form(app: &App) {
    ...
    state.set_selected_dto_out_field_optional(false);
    state.set_selected_dto_out_field_is_list(false);
    state.set_selected_dto_out_field_enum_name("".into());
    state.set_selected_dto_out_field_enum_values("".into());
}
...
pub fn setup_dto_out_field_selected_callback(app: &App, app_context: &Arc<AppContext>) {
    app.global::<FeaturesTabState>().on_dto_out_field_selected({
        ...
        move |field_id| {
            ...
            if field_id <= 0 {
                log::warn!("Ignored invalid DTO Out field selection id: {}", field_id);
                return;
            }
            if let Some(app) = app_weak.upgrade() {
                let field_res =
...
pub fn setup_dto_out_field_optional_callback(app: &App, app_context: &Arc<AppContext>) {
    app.global::<FeaturesTabState>()
        .on_dto_out_field_optional_changed({
            let ctx = Arc::clone(app_context);
            let app_weak = app.as_weak();
            move |optional| {
                if let Some(app) = app_weak.upgrade() {
...
pub fn setup_dto_out_field_enum_name_callback(app: &App, app_context: &Arc<AppContext>) {
    app.global::<FeaturesTabState>()
        .on_dto_out_field_enum_name_changed({
            let ctx = Arc::clone(app_context);
            let app_weak = app.as_weak();
            move |name| {
                if let Some(app) = app_weak.upgrade() {
...
pub fn setup_dto_out_field_enum_values_callback(app: &App, app_context: &Arc<AppContext>) {
    app.global::<FeaturesTabState>()
        .on_dto_out_field_enum_values_changed({
            let ctx = Arc::clone(app_context);
            let app_weak = app.as_weak();
            move |values| {
                if let Some(app) = app_weak.upgrade() {
