// Generated by Qleany. Edit at your own risk! Be careful when regenerating this file
// as changes will be lost.
use crate::use_cases::common::rust_code_generator::{
    GenerationOps, GenerationSnapshot, SnapshotBuilder, generate_code_with_snapshot,
};
use crate::use_cases::common::rust_formatter::rustfmt_files_batch;
use crate::use_cases::common::tools;
use anyhow::{Result, anyhow};
use common::database::CommandUnitOfWork;
use common::direct_access::system::SystemRelationshipField;
use common::entities::File;
use common::long_operation::LongOperation;
use common::types::EntityId;
use rayon::prelude::*;
use std::path::PathBuf;
use std::sync::Arc;

pub trait FillCodeInRustFilesUnitOfWorkFactoryTrait: Send + Sync {
    fn create(&self) -> Box<dyn FillCodeInRustFilesUnitOfWorkTrait>;
}

// Exactly the same macros must be set in ../units_of_work/fill_code_in_rust_files_uow.rs
#[macros::uow_action(entity = "System", action = "GetRelationship")]
#[macros::uow_action(entity = "File", action = "GetMulti")]
#[macros::uow_action(entity = "File", action = "UpdateMulti")]
#[macros::uow_action(entity = "File", action = "RemoveMulti")]
pub trait FillCodeInRustFilesUnitOfWorkTrait:
    CommandUnitOfWork + GenerationOps + Send + Sync
{
}

pub struct FillCodeInRustFilesUseCase {
    uow_factory: Box<dyn FillCodeInRustFilesUnitOfWorkFactoryTrait>,
}

impl FillCodeInRustFilesUseCase {
    pub fn new(uow_factory: Box<dyn FillCodeInRustFilesUnitOfWorkFactoryTrait>) -> Self {
        FillCodeInRustFilesUseCase { uow_factory }
    }
}
impl LongOperation for FillCodeInRustFilesUseCase {
    type Output = ();
    fn execute(
        &self,
        progress_callback: Box<dyn Fn(common::long_operation::OperationProgress) + Send>,
        cancel_flag: Arc<std::sync::atomic::AtomicBool>,
    ) -> Result<()> {
        use std::fs;
        use std::sync::atomic::Ordering;

        let start_time = std::time::Instant::now();

        progress_callback(common::long_operation::OperationProgress::new(
            0.0,
            Some("Starting code generation for Rust files...".to_string()),
        ));

        let mut uow = self.uow_factory.create();
        uow.begin_transaction()?;

        // Get system ID and all file IDs
        let uow_ops: &dyn GenerationOps = &*uow;
        let system_id = tools::get_system_id(uow_ops)?;
        let file_ids = uow.get_system_relationship(&system_id, &SystemRelationshipField::Files)?;

        if file_ids.is_empty() {
            uow.commit()?;
            progress_callback(common::long_operation::OperationProgress::new(
                100.0,
                Some("No files to process".to_string()),
            ));
            return Ok(());
        }

        // Load all file metadata
        let files: Vec<File> = uow
            .get_file_multi(&file_ids)?
            .into_iter()
            .flatten()
            .collect();

        let total = files.len().max(1);

        // Create a temp directory that mirrors the real folder structure so
        // rustfmt can resolve `mod` references when formatting.
        let tmp_dir = std::env::temp_dir().join(format!("qleany_fill_code_{}", std::process::id()));
        fs::create_dir_all(&tmp_dir)?;

        // Phase 1 (sequential): Build all snapshots via UoW (DB-bound)
        let mut generation_snapshot_cache: Vec<GenerationSnapshot> =
            Vec::with_capacity(files.len());

        let mut file_snapshots: Vec<(&File, GenerationSnapshot)> = Vec::with_capacity(files.len());

        for (idx, file) in files.iter().enumerate() {
            if cancel_flag.load(Ordering::Relaxed) {
                let _ = fs::remove_dir_all(&tmp_dir);
                uow.rollback()?;
                uow.begin_transaction()?;
                if !file_ids.is_empty() {
                    uow.remove_file_multi(&file_ids)?;
                }
                uow.commit()?;
                return Err(anyhow!("Operation was cancelled"));
            }

            let uow_ops: &dyn GenerationOps = &*uow;
            let (snapshot, from_cache) =
                SnapshotBuilder::for_file_id(uow_ops, file.id, &generation_snapshot_cache)?;
            if !from_cache {
                generation_snapshot_cache.push(snapshot.clone());
            }
            file_snapshots.push((file, snapshot));

            let percentage = ((idx + 1) as f32 / total as f32) * 40.0;
            progress_callback(common::long_operation::OperationProgress::new(
                percentage,
                Some(format!(
                    "Snapshot {}/{}: {}{}",
                    idx + 1,
                    total,
                    file.relative_path,
                    file.name
                )),
            ));
        }

        // Phase 2 (parallel): Render templates + write files (CPU+IO-bound)
        let generated: Vec<(EntityId, String, PathBuf)> = file_snapshots
            .par_iter()
            .map(|(file, snapshot)| {
                if cancel_flag.load(Ordering::Relaxed) {
                    return Err(anyhow!("Operation was cancelled"));
                }

                let code = generate_code_with_snapshot(snapshot)?;

                let file_dir = tmp_dir.join(&file.relative_path);
                fs::create_dir_all(&file_dir)?;
                let temp_path = file_dir.join(&file.name);
                fs::write(&temp_path, code.as_bytes())?;

                Ok((file.id, code, temp_path))
            })
            .collect::<Result<Vec<_>>>()?;

        // If cancelled during parallel phase, clean up
        if cancel_flag.load(Ordering::Relaxed) {
            let _ = fs::remove_dir_all(&tmp_dir);
            uow.rollback()?;
            uow.begin_transaction()?;
            if !file_ids.is_empty() {
                uow.remove_file_multi(&file_ids)?;
            }
            uow.commit()?;
            return Err(anyhow!("Operation was cancelled"));
        }

        let rust_files_to_format: Vec<PathBuf> = generated
            .iter()
            .filter(|(_, _, path)| path.extension().is_some_and(|ext| ext == "rs"))
            .map(|(_, _, path)| path.clone())
            .collect();

        progress_callback(common::long_operation::OperationProgress::new(
            80.0,
            Some(format!("Generated {}/{} files", generated.len(), total)),
        ));

        // Batch format all Rust files at once
        if !rust_files_to_format.is_empty() {
            progress_callback(common::long_operation::OperationProgress::new(
                82.0,
                Some(format!(
                    "Formatting {} Rust files...",
                    rust_files_to_format.len()
                )),
            ));
            rustfmt_files_batch(&rust_files_to_format);
        }

        // Read back formatted files and update entities
        progress_callback(common::long_operation::OperationProgress::new(
            90.0,
            Some("Updating file entities with generated code...".to_string()),
        ));

        let mut files_to_update: Vec<File> = Vec::new();
        for (idx, (file_id, fallback_code, temp_path)) in generated.iter().enumerate() {
            if cancel_flag.load(Ordering::Relaxed) {
                let _ = fs::remove_dir_all(&tmp_dir);
                uow.rollback()?;
                // Delete all File entities created by the prior fill_files step
                uow.begin_transaction()?;
                if !file_ids.is_empty() {
                    uow.remove_file_multi(&file_ids)?;
                }
                uow.commit()?;
                return Err(anyhow!("Operation was cancelled"));
            }

            // Read back the (potentially formatted) content
            let final_code =
                fs::read_to_string(temp_path).unwrap_or_else(|_| fallback_code.clone());

            // Update file entity with the generated code
            let mut file = files
                .iter()
                .find(|f| f.id == *file_id)
                .cloned()
                .ok_or_else(|| anyhow!("File entity {} not found", file_id))?;
            file.generated_code = Some(final_code);
            file.updated_at = chrono::Utc::now();
            files_to_update.push(file);

            // Progress: update phase is 90-99%
            let percentage = 90.0 + ((idx + 1) as f32 / generated.len() as f32) * 9.0;
            progress_callback(common::long_operation::OperationProgress::new(
                percentage,
                Some(format!("Updated {}/{}", idx + 1, generated.len())),
            ));
        }
        uow.update_file_multi(files_to_update.as_slice())?;

        // Clean up temp directory
        let _ = fs::remove_dir_all(&tmp_dir);

        uow.commit()?;

        let duration = start_time.elapsed();
        log::debug!(
            "Fill code in Rust files completed in {:?}, total files: {}",
            duration,
            files.len()
        );

        progress_callback(common::long_operation::OperationProgress::new(
            100.0,
            Some("completed".to_string()),
        ));
        Ok(())
    }
}
