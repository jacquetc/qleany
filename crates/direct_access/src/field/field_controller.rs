...
use super::{
    dtos::{CreateFieldDto, FieldDto, UpdateFieldDto},
    units_of_work::{FieldReadUoWFactory, FieldWriteUoWFactory},
};
...
pub fn create_orphan(
    ...
    let uow_factory = FieldWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::Field = entity.into();
...
pub fn create_orphan_multi(
    ...
    let uow_factory = FieldWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::Field> = entities.iter().map(|dto| dto.into()).collect();
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<FieldDto>> {
    let uow_factory = FieldReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<FieldDto>> {
    let uow_factory = FieldReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
...
pub fn get_relationship(
    ...
    let uow_factory = FieldReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipUseCase::<FieldRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_many(
    ...
    let uow_factory = FieldReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipManyUseCase::<FieldRelationshipField, _>::new(uow_factory);
...
pub fn get_relationship_count(
    ...
    let uow_factory = FieldReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipCountUseCase::<FieldRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_in_range(
    ...
    let uow_factory = FieldReadUoWFactory::new(db_context);
    let uc =
        use_cases::GetRelationshipInRangeUseCase::<FieldRelationshipField, _>::new(uow_factory);
...
pub fn set_relationship(
    ...
    let uow_factory = FieldWriteUoWFactory::new(db_context, event_hub);
    let mut uc =
        use_cases::UndoableSetRelationshipUseCase::<FieldRelationshipField, _>::new(uow_factory);
...
pub fn move_relationship(
    ...
    let uow_factory = FieldWriteUoWFactory::new(db_context, event_hub);
    let mut uc =
        use_cases::UndoableMoveRelationshipUseCase::<FieldRelationshipField, _>::new(uow_factory);
...
mod tests {
    ...
    use super::*;
    use crate::entity::dtos::CreateEntityDto;
    ...
    fn create_owner_chain(ctx: &mut TestContext) -> EntityId {
        ...
        .unwrap();
        let entity = entity_controller::create(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
            &CreateEntityDto::default(),
            workspace.id,
            -1,
        )
        .unwrap();
    ...
    fn create_one(ctx: &mut TestContext) -> FieldDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
            &CreateFieldDto::default(),
        )
        .unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateFieldDto = created.into();
    ...
    fn test_get_relationship_default() {
        ...
        let created = create_one(&mut ctx);
        let rel_ids =
            get_relationship(&ctx.db, &created.id, &FieldRelationshipField::Entity).unwrap();
    ...
    fn test_get_relationship_count_default() {
        ...
        let created = create_one(&mut ctx);
        let count =
            get_relationship_count(&ctx.db, &created.id, &FieldRelationshipField::Entity).unwrap();
        assert_eq!(count, 0);
    }
}
