...
impl CreateOrphanFieldUseCase {
    ...
    pub fn execute(&mut self, dto: CreateFieldDto) -> Result<FieldDto> {
        ...
        let entity = uow.create_orphan_field(&dto.into())?;
        uow.commit()?;
        // store in undo stack
...
impl UndoRedoCommand for CreateOrphanFieldUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_field(&last_entity)?;
            uow.commit()?;
