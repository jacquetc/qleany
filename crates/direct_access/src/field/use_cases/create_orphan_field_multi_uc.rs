...
impl CreateOrphanFieldMultiUseCase {
    ...
    pub fn execute(&mut self, dtos: &[CreateFieldDto]) -> Result<Vec<FieldDto>> {
        ...
        let entities =
            uow.create_orphan_field_multi(&dtos.iter().map(|dto| dto.into()).collect::<Vec<_>>())?;
        uow.commit()?;
        //store in undo stack
...
impl UndoRedoCommand for CreateOrphanFieldMultiUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entities) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_field_multi(&last_entities)?;
            uow.commit()?;
