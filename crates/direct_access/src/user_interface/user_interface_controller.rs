...
use super::{
    dtos::{CreateUserInterfaceDto, UpdateUserInterfaceDto, UserInterfaceDto},
    units_of_work::{UserInterfaceReadUoWFactory, UserInterfaceWriteUoWFactory},
};
...
pub fn create_orphan(
    ...
    let uow_factory = UserInterfaceWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::UserInterface = entity.into();
...
pub fn create_orphan_multi(
    ...
    let uow_factory = UserInterfaceWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::UserInterface> =
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<UserInterfaceDto>> {
    let uow_factory = UserInterfaceReadUoWFactory::new(db_context);
...
pub fn update_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[UpdateUserInterfaceDto],
) -> Result<Vec<UserInterfaceDto>> {
    let uow_factory = UserInterfaceWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::UserInterface> =
        entities.iter().map(|dto| dto.into()).collect();
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> UserInterfaceDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
            &CreateUserInterfaceDto::default(),
        )
        .unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateUserInterfaceDto = created.into();
