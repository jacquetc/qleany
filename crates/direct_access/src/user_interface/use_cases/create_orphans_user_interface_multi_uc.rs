...
pub struct CreateOrphansUserInterfaceMultiUseCase {
...
impl CreateOrphansUserInterfaceMultiUseCase {
    pub fn new(uow_factory: Box<dyn UserInterfaceUnitOfWorkFactoryTrait>) -> Self {
        CreateOrphansUserInterfaceMultiUseCase {
            uow_factory,
    ...
    pub fn execute(&mut self, dtos: &[CreateUserInterfaceDto]) -> Result<Vec<UserInterfaceDto>> {
        ...
        let entities = uow
            .create_user_interface_multi(&dtos.iter().map(|dto| dto.into()).collect::<Vec<_>>())?;
        uow.commit()?;
        //store in undo stack
...
impl UndoRedoCommand for CreateOrphansUserInterfaceMultiUseCase {
