// Generated by Qleany. Edit at your own risk! Be careful when regenerating this file
// as changes will be lost.

use super::UserInterfaceUnitOfWorkFactoryTrait;
use crate::user_interface::dtos::{CreateUserInterfaceDto, UserInterfaceDto};
use anyhow::{Ok, Result};
use common::entities::UserInterface;
use common::undo_redo::UndoRedoCommand;
use std::any::Any;
use std::collections::VecDeque;

pub struct CreateOrphanUserInterfaceUseCase {
    uow_factory: Box<dyn UserInterfaceUnitOfWorkFactoryTrait>,
    undo_stack: VecDeque<UserInterface>,
    redo_stack: VecDeque<UserInterface>,
}

impl CreateOrphanUserInterfaceUseCase {
    pub fn new(uow_factory: Box<dyn UserInterfaceUnitOfWorkFactoryTrait>) -> Self {
        CreateOrphanUserInterfaceUseCase {
            uow_factory,
            undo_stack: VecDeque::new(),
            redo_stack: VecDeque::new(),
        }
    }

    pub fn execute(&mut self, dto: CreateUserInterfaceDto) -> Result<UserInterfaceDto> {
        let mut uow = self.uow_factory.create();
        uow.begin_transaction()?;
        let entity = uow.create_orphan_user_interface(&dto.into())?;
        uow.commit()?;
        // store in undo stack
        self.undo_stack.push_back(entity.clone());
        self.redo_stack.clear();

        Ok(entity.into())
    }
}
impl UndoRedoCommand for CreateOrphanUserInterfaceUseCase {
    fn undo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.undo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.delete_user_interface(&last_entity.id)?;
            uow.commit()?;
            self.redo_stack.push_back(last_entity);
        }
        Ok(())
    }

    fn redo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_user_interface(&last_entity)?;
            uow.commit()?;
            self.undo_stack.push_back(last_entity);
        }
        Ok(())
    }
    fn as_any(&self) -> &dyn Any {
        self
    }
}
