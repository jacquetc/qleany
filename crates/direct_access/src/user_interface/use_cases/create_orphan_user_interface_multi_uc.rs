...
pub struct CreateOrphanUserInterfaceMultiUseCase {
...
impl CreateOrphanUserInterfaceMultiUseCase {
    pub fn new(uow_factory: Box<dyn UserInterfaceUnitOfWorkFactoryTrait>) -> Self {
        CreateOrphanUserInterfaceMultiUseCase {
            uow_factory,
    ...
    pub fn execute(&mut self, dtos: &[CreateUserInterfaceDto]) -> Result<Vec<UserInterfaceDto>> {
        ...
        let entities = uow.create_orphan_user_interface_multi(
...
impl UndoRedoCommand for CreateOrphanUserInterfaceMultiUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entities) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_user_interface_multi(&last_entities)?;
            uow.commit()?;
