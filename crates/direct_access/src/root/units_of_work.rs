...
impl RootUnitOfWorkTrait for RootUnitOfWork {
    ...
    // Function is now generated by the macro
    ...
    //         &self.transaction.as_ref().expect("Transaction not started"),
    //     );
    //     let value = root_repo.get_relationships_from_right_ids(field, right_ids)?;
    //     Ok(value)
    // }
    ...
    //         &self.transaction.as_ref().expect("Transaction not started"),
    //     );
    //     root_repo.set_relationship_multi(&self.event_hub, field, relationships)?;
    //     Ok(())
    // }
}
