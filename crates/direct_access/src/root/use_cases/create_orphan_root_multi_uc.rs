...
pub struct CreateOrphanRootMultiUseCase {
    uow_factory: Box<dyn RootUnitOfWorkFactoryTrait>,
}

impl CreateOrphanRootMultiUseCase {
    pub fn new(uow_factory: Box<dyn RootUnitOfWorkFactoryTrait>) -> Self {
        CreateOrphanRootMultiUseCase { uow_factory }
    }
    pub fn execute(&mut self, dtos: &[CreateRootDto]) -> Result<Vec<RootDto>> {
        ...
        let entities =
            uow.create_orphan_root_multi(&dtos.iter().map(|dto| dto.into()).collect::<Vec<_>>())?;
        uow.commit()?;
