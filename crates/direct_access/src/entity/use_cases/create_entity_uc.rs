...
impl CreateEntityUseCase {
    ...
    pub fn execute(
        ...
        let entity = uow.create_entity_with_owner(&dto.into(), owner_id, index)?;
...
impl UndoRedoCommand for CreateEntityUseCase {
    fn undo(&mut self) -> Result<()> {
        if let Some(ref created) = self.created_entity {
            ...
            self.old_tree_snapshot = Some(uow.snapshot_entity(&[created.id])?);

            // Remove the created entity
            uow.delete_entity(&created.id)?;
