...
impl CreateOrphanEntityUseCase {
    ...
    pub fn execute(&mut self, dto: CreateEntityDto) -> Result<EntityDto> {
        ...
        let entity = uow.create_orphan_entity(&dto.into())?;
        uow.commit()?;
        // store in undo stack
...
impl UndoRedoCommand for CreateOrphanEntityUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_entity(&last_entity)?;
            uow.commit()?;
