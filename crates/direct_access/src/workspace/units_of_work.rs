...
impl CommandUnitOfWork for WorkspaceWriteUoW {
...
impl use_cases::WriteUoW for WorkspaceWriteUoW {
    type Entity = Workspace;

    fn get(&self, id: &EntityId) -> Result<Option<Workspace>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_workspace_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Workspace>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_workspace_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<Workspace>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_workspace_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[Workspace]) -> Result<Vec<Workspace>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_workspace_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Workspace]) -> Result<Vec<Workspace>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_workspace_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Workspace]) -> Result<Vec<Workspace>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_workspace_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for WorkspaceWriteUoW {
    fn create_multi(
        &self,
        entities: &[Workspace],
        owner_id: EntityId,
...
impl use_cases::WriteRelUoW<WorkspaceRelationshipField> for WorkspaceWriteUoW {
    fn set_relationship(
        &self,
        id: &EntityId,
...
pub struct WorkspaceWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl WorkspaceWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        WorkspaceWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for WorkspaceWriteUoWFactory {
    type Entity = Workspace;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Workspace>> {
        Box::new(WorkspaceWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for WorkspaceWriteUoWFactory {
    type Entity = Workspace;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Workspace>> {
        Box::new(WorkspaceWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<WorkspaceRelationshipField> for WorkspaceWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<WorkspaceRelationshipField>> {
        Box::new(WorkspaceWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
pub struct WorkspaceReadUoW {
    context: DbContext,
    transaction: RefCell<Option<Transaction>>,
}

impl WorkspaceReadUoW {
...
impl QueryUnitOfWork for WorkspaceReadUoW {
...
impl use_cases::ReadUoW for WorkspaceReadUoW {
    type Entity = Workspace;

    fn get(&self, id: &EntityId) -> Result<Option<Workspace>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_workspace_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<WorkspaceRelationshipField> for WorkspaceReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &WorkspaceRelationshipField,
...
pub struct WorkspaceReadUoWFactory {
    context: DbContext,
}

impl WorkspaceReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        WorkspaceReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for WorkspaceReadUoWFactory {
    type Entity = Workspace;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Workspace>> {
        Box::new(WorkspaceReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<WorkspaceRelationshipField> for WorkspaceReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<WorkspaceRelationshipField>> {
        Box::new(WorkspaceReadUoW::new(&self.context))
    }
}
