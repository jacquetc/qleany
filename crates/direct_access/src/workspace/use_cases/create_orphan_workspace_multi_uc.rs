...
impl CreateOrphanWorkspaceMultiUseCase {
    ...
    pub fn execute(&mut self, dtos: &[CreateWorkspaceDto]) -> Result<Vec<WorkspaceDto>> {
        ...
        let entities = uow.create_orphan_workspace_multi(
...
impl UndoRedoCommand for CreateOrphanWorkspaceMultiUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entities) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_workspace_multi(&last_entities)?;
            uow.commit()?;
