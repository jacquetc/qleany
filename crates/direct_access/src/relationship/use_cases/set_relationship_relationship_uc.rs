...
pub struct SetRelationshipRelationshipUseCase {
    uow_factory: Box<dyn RelationshipUnitOfWorkFactoryTrait>,
    undo_stack: VecDeque<Savepoint>,
    redo_stack: VecDeque<RelationshipRelationshipDto>,
}

impl SetRelationshipRelationshipUseCase {
    pub fn new(uow_factory: Box<dyn RelationshipUnitOfWorkFactoryTrait>) -> Self {
        SetRelationshipRelationshipUseCase {
            uow_factory,
    ...
    pub fn execute(&mut self, dto: &RelationshipRelationshipDto) -> Result<()> {
        let mut uow = self.uow_factory.create();
        uow.begin_transaction()?;
        let savepoint = uow.create_savepoint()?;
        uow.set_relationship_relationship(&dto.id, &dto.field, dto.right_ids.as_slice())?;
        uow.commit()?;
...
impl UndoRedoCommand for SetRelationshipRelationshipUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(RelationshipRelationshipDto {
            id,
            field,
            right_ids,
        }) = self.redo_stack.pop_back()
        {
            ...
            uow.begin_transaction()?;
            let savepoint = uow.create_savepoint()?;
            uow.set_relationship_relationship(&id, &field, &right_ids)?;
            uow.commit()?;
            self.undo_stack.push_back(savepoint);
        }
        anyhow::Ok(())
