...
impl CreateOrphanRelationshipUseCase {
    ...
    pub fn execute(&mut self, dto: CreateRelationshipDto) -> Result<RelationshipDto> {
        ...
        let entity = uow.create_orphan_relationship(&dto.into())?;
        uow.commit()?;
        // store in undo stack
...
impl UndoRedoCommand for CreateOrphanRelationshipUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_relationship(&last_entity)?;
            uow.commit()?;
