...
pub struct CreateOrphanRelationshipMultiUseCase {
...
impl CreateOrphanRelationshipMultiUseCase {
    pub fn new(uow_factory: Box<dyn RelationshipUnitOfWorkFactoryTrait>) -> Self {
        CreateOrphanRelationshipMultiUseCase {
            uow_factory,
    ...
    pub fn execute(&mut self, dtos: &[CreateRelationshipDto]) -> Result<Vec<RelationshipDto>> {
        ...
        let entities = uow.create_orphan_relationship_multi(
...
impl UndoRedoCommand for CreateOrphanRelationshipMultiUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entities) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_relationship_multi(&last_entities)?;
            uow.commit()?;
