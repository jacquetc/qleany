...
impl CommandUnitOfWork for GlobalWriteUoW {
...
impl use_cases::WriteUoW for GlobalWriteUoW {
    type Entity = Global;

    fn get(&self, id: &EntityId) -> Result<Option<Global>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_global_repository(transaction);
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Global>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_global_repository(transaction);
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<Global>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_global_repository(transaction);
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[Global]) -> Result<Vec<Global>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_global_repository(transaction);
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Global]) -> Result<Vec<Global>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_global_repository(transaction);
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for GlobalWriteUoW {
    fn create_multi(
        &self,
        entities: &[Global],
        owner_id: EntityId,
...
pub struct GlobalWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl GlobalWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        GlobalWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for GlobalWriteUoWFactory {
    type Entity = Global;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Global>> {
        Box::new(GlobalWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for GlobalWriteUoWFactory {
    type Entity = Global;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Global>> {
        Box::new(GlobalWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for GlobalReadUoW {
...
impl use_cases::ReadUoW for GlobalReadUoW {
    type Entity = Global;

    fn get(&self, id: &EntityId) -> Result<Option<Global>> {
        let transaction = self.transaction.borrow();
        let repo =
            repository_factory::read::create_global_repository(transaction.as_ref().unwrap());
        Ok(repo.get(id)?)
    }
...
pub struct GlobalReadUoWFactory {
    context: DbContext,
}

impl GlobalReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        GlobalReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for GlobalReadUoWFactory {
    type Entity = Global;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Global>> {
        Box::new(GlobalReadUoW::new(&self.context))
    }
}
