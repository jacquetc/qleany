...
impl CreateOrphanGlobalUseCase {
    ...
    pub fn execute(&mut self, dto: CreateGlobalDto) -> Result<GlobalDto> {
        ...
        let entity = uow.create_orphan_global(&dto.into())?;
        uow.commit()?;
        // store in undo stack
...
impl UndoRedoCommand for CreateOrphanGlobalUseCase {
    fn undo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.undo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.delete_global(&last_entity.id)?;
            uow.commit()?;
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_global(&last_entity)?;
            uow.commit()?;
