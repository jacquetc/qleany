...
pub struct GetGlobalUseCase<'a> {
    uow: &'a dyn GlobalUnitOfWorkTraitRO,
}

impl<'a> GetGlobalUseCase<'a> {
    pub fn new(uow: &'a dyn GlobalUnitOfWorkTraitRO) -> Self {
        GetGlobalUseCase { uow }
    }

    pub fn execute(&self, id: &EntityId) -> Result<Option<GlobalDto>> {
        self.uow.begin_transaction()?;
        let global_option = self.uow.get_global(&id)?;
