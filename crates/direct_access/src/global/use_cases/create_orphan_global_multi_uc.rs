...
impl CreateOrphanGlobalMultiUseCase {
    ...
    pub fn execute(&mut self, dtos: &[CreateGlobalDto]) -> Result<Vec<GlobalDto>> {
        ...
        let entities =
            uow.create_orphan_global_multi(&dtos.iter().map(|dto| dto.into()).collect::<Vec<_>>())?;
        uow.commit()?;
        //store in undo stack
...
impl UndoRedoCommand for CreateOrphanGlobalMultiUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entities) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_global_multi(&last_entities)?;
            uow.commit()?;
