...
use super::{
    dtos::{CreateGlobalDto, GlobalDto, UpdateGlobalDto},
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> GlobalDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
            &CreateGlobalDto::default(),
        )
        .unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateGlobalDto = created.into();
