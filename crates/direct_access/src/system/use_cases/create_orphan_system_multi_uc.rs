...
pub struct CreateOrphanSystemMultiUseCase {
    uow_factory: Box<dyn SystemUnitOfWorkFactoryTrait>,
}

impl CreateOrphanSystemMultiUseCase {
    pub fn new(uow_factory: Box<dyn SystemUnitOfWorkFactoryTrait>) -> Self {
        CreateOrphanSystemMultiUseCase { uow_factory }
    }
    pub fn execute(&mut self, dtos: &[CreateSystemDto]) -> Result<Vec<SystemDto>> {
        ...
        let entities =
            uow.create_orphan_system_multi(&dtos.iter().map(|dto| dto.into()).collect::<Vec<_>>())?;
        uow.commit()?;
