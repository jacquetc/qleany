...
impl UndoRedoCommand for SetSystemRelationshipUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(SystemRelationshipDto {
            id,
            field,
            right_ids,
        }) = self.redo_stack.pop_back()
        {
            ...
            uow.begin_transaction()?;
            let savepoint = uow.create_savepoint()?;
            uow.set_system_relationship(&id, &field, &right_ids)?;
            uow.commit()?;
            self.undo_stack.push_back(savepoint);
        }
        anyhow::Ok(())
