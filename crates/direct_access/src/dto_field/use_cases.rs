// Generated by Qleany. Edit at your own risk! Be careful when regenerating this file
// as changes will be lost.


// Sub-modules for DtoField use cases
pub(super) mod get_dto_field_uc;
pub(super) mod get_dto_field_multi_uc;
pub(super) mod get_all_dto_field_uc;
pub(super) mod create_orphan_dto_field_multi_uc;
pub(super) mod create_orphan_dto_field_uc;pub(super) mod create_dto_field_multi_uc;
pub(super) mod create_dto_field_uc;pub(super) mod update_dto_field_multi_uc;
pub(super) mod update_dto_field_uc;
pub(super) mod remove_dto_field_multi_uc;
pub(super) mod remove_dto_field_uc;

use anyhow::Result;
use common::database::{CommandUnitOfWork, QueryUnitOfWork};
use common::entities::DtoField;
use common::types::EntityId;

pub(in crate::dto_field) trait DtoFieldUnitOfWorkFactoryTrait: Send + Sync {
    fn create(&self) -> Box<dyn DtoFieldUnitOfWorkTrait>;
}

#[macros::uow_action(entity = "DtoField", action = "CreateOrphan")]
#[macros::uow_action(entity = "DtoField", action = "CreateOrphanMulti")]
#[macros::uow_action(entity = "DtoField", action = "Create")]
#[macros::uow_action(entity = "DtoField", action = "CreateMulti")]
#[macros::uow_action(entity = "DtoField", action = "Get")]
#[macros::uow_action(entity = "DtoField", action = "GetMulti")]
#[macros::uow_action(entity = "DtoField", action = "GetAll")]
#[macros::uow_action(entity = "DtoField", action = "Update")]
#[macros::uow_action(entity = "DtoField", action = "UpdateMulti")]
#[macros::uow_action(entity = "DtoField", action = "Delete")]
#[macros::uow_action(entity = "DtoField", action = "DeleteMulti")]
#[macros::uow_action(entity = "DtoField", action = "Snapshot")]
#[macros::uow_action(entity = "DtoField", action = "Restore")]
pub(in crate::dto_field) trait DtoFieldUnitOfWorkTrait: CommandUnitOfWork {
    fn get_relationships_from_owner(&self, owner_id: &EntityId) -> Result<Vec<EntityId>>;
    fn set_relationships_in_owner(&self, owner_id: &EntityId, ids: &[EntityId]) -> Result<()>;
}

pub(in crate::dto_field) trait DtoFieldUnitOfWorkROFactoryTrait {
    fn create(&self) -> Box<dyn DtoFieldUnitOfWorkROTrait>;
}

#[macros::uow_action(entity = "DtoField", action = "GetRO")]
#[macros::uow_action(entity = "DtoField", action = "GetMultiRO")]
#[macros::uow_action(entity = "DtoField", action = "GetAllRO")]
pub(in crate::dto_field) trait DtoFieldUnitOfWorkROTrait: QueryUnitOfWork {}
