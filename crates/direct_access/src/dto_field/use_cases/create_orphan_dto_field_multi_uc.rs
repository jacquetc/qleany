...
impl CreateOrphanDtoFieldMultiUseCase {
    ...
    pub fn execute(&mut self, dtos: &[CreateDtoFieldDto]) -> Result<Vec<DtoFieldDto>> {
        ...
        let entities = uow.create_orphan_dto_field_multi(
...
impl UndoRedoCommand for CreateOrphanDtoFieldMultiUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entities) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_dto_field_multi(&last_entities)?;
            uow.commit()?;
