// Generated by Qleany. Edit at your own risk! Be careful when regenerating this file
// as changes will be lost.

use super::DtoFieldUnitOfWorkFactoryTrait;
use crate::dto_field::dtos::{CreateDtoFieldDto, DtoFieldDto};
use anyhow::{Ok, Result};
use common::entities::DtoField;
use common::undo_redo::UndoRedoCommand;
use std::any::Any;
use std::collections::VecDeque;

pub struct CreateOrphanDtoFieldUseCase {
    uow_factory: Box<dyn DtoFieldUnitOfWorkFactoryTrait>,
    undo_stack: VecDeque<DtoField>,
    redo_stack: VecDeque<DtoField>,
}

impl CreateOrphanDtoFieldUseCase {
    pub fn new(uow_factory: Box<dyn DtoFieldUnitOfWorkFactoryTrait>) -> Self {
        CreateOrphanDtoFieldUseCase {
            uow_factory,
            undo_stack: VecDeque::new(),
            redo_stack: VecDeque::new(),
        }
    }

    pub fn execute(&mut self, dto: CreateDtoFieldDto) -> Result<DtoFieldDto> {
        let mut uow = self.uow_factory.create();
        uow.begin_transaction()?;
        let entity = uow.create_orphan_dto_field(&dto.into())?;
        uow.commit()?;
        // store in undo stack
        self.undo_stack.push_back(entity.clone());
        self.redo_stack.clear();

        Ok(entity.into())
    }
}
impl UndoRedoCommand for CreateOrphanDtoFieldUseCase {
    fn undo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.undo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.delete_dto_field(&last_entity.id)?;
            uow.commit()?;
            self.redo_stack.push_back(last_entity);
        }
        Ok(())
    }

    fn redo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_dto_field(&last_entity)?;
            uow.commit()?;
            self.undo_stack.push_back(last_entity);
        }
        Ok(())
    }
    fn as_any(&self) -> &dyn Any {
        self
    }
}
