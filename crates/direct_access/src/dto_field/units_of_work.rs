...
impl CommandUnitOfWork for DtoFieldWriteUoW {
...
impl use_cases::WriteUoW for DtoFieldWriteUoW {
    type Entity = DtoField;

    fn get(&self, id: &EntityId) -> Result<Option<DtoField>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_dto_field_repository(transaction)?;
        Ok(repo.get(id)?)
    }
    ...
    fn create_orphan_multi(&self, entities: &[DtoField]) -> Result<Vec<DtoField>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_dto_field_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[DtoField]) -> Result<Vec<DtoField>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_dto_field_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[DtoField]) -> Result<Vec<DtoField>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_dto_field_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for DtoFieldWriteUoW {
    fn create_multi(
        &self,
        entities: &[DtoField],
        owner_id: EntityId,
...
pub struct DtoFieldWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl DtoFieldWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        DtoFieldWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for DtoFieldWriteUoWFactory {
    type Entity = DtoField;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = DtoField>> {
        Box::new(DtoFieldWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for DtoFieldWriteUoWFactory {
    type Entity = DtoField;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = DtoField>> {
        Box::new(DtoFieldWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
pub struct DtoFieldReadUoW {
    context: DbContext,
    transaction: RefCell<Option<Transaction>>,
}

impl DtoFieldReadUoW {
...
impl QueryUnitOfWork for DtoFieldReadUoW {
...
impl use_cases::ReadUoW for DtoFieldReadUoW {
    type Entity = DtoField;
...
pub struct DtoFieldReadUoWFactory {
    context: DbContext,
}

impl DtoFieldReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        DtoFieldReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for DtoFieldReadUoWFactory {
    type Entity = DtoField;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = DtoField>> {
        Box::new(DtoFieldReadUoW::new(&self.context))
    }
}
