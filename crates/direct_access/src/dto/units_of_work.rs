...
impl CommandUnitOfWork for DtoWriteUoW {
...
impl use_cases::WriteUoW for DtoWriteUoW {
    type Entity = Dto;

    fn get(&self, id: &EntityId) -> Result<Option<Dto>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_dto_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Dto>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_dto_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<Dto>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_dto_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[Dto]) -> Result<Vec<Dto>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_dto_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Dto]) -> Result<Vec<Dto>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_dto_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Dto]) -> Result<Vec<Dto>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_dto_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for DtoWriteUoW {
    fn create_multi(&self, entities: &[Dto], owner_id: EntityId, index: i32) -> Result<Vec<Dto>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_dto_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::WriteRelUoW<DtoRelationshipField> for DtoWriteUoW {
    fn set_relationship(
        &self,
        id: &EntityId,
...
pub struct DtoWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl DtoWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        DtoWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
...
impl use_cases::WriteUoWFactory for DtoWriteUoWFactory {
    type Entity = Dto;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Dto>> {
        Box::new(DtoWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for DtoWriteUoWFactory {
    type Entity = Dto;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Dto>> {
        Box::new(DtoWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<DtoRelationshipField> for DtoWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<DtoRelationshipField>> {
        Box::new(DtoWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for DtoReadUoW {
...
impl use_cases::ReadUoW for DtoReadUoW {
    type Entity = Dto;

    fn get(&self, id: &EntityId) -> Result<Option<Dto>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_dto_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<DtoRelationshipField> for DtoReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &DtoRelationshipField,
    ...
    fn get_relationship_count(&self, id: &EntityId, field: &DtoRelationshipField) -> Result<usize> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_dto_repository(
            transaction
                .as_ref()
...
pub struct DtoReadUoWFactory {
    context: DbContext,
}

impl DtoReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        DtoReadUoWFactory {
            context: db_context.clone(),
        }
...
impl use_cases::ReadUoWFactory for DtoReadUoWFactory {
    type Entity = Dto;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Dto>> {
        Box::new(DtoReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<DtoRelationshipField> for DtoReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<DtoRelationshipField>> {
        Box::new(DtoReadUoW::new(&self.context))
    }
}
