...
impl SetDtoRelationshipUseCase {
    ...
    pub fn execute(&mut self, dto: &DtoRelationshipDto) -> Result<()> {
        let mut uow = self.uow_factory.create();
        uow.begin_transaction()?;
        let savepoint = uow.create_savepoint()?;
        uow.set_dto_relationship(&dto.id, &dto.field, dto.right_ids.as_slice())?;
        uow.commit()?;
...
impl UndoRedoCommand for SetDtoRelationshipUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(DtoRelationshipDto {
            id,
            field,
            right_ids,
        }) = self.redo_stack.pop_back()
        {
            ...
            uow.begin_transaction()?;
            let savepoint = uow.create_savepoint()?;
            uow.set_dto_relationship(&id, &field, &right_ids)?;
            uow.commit()?;
            self.undo_stack.push_back(savepoint);
        }
        anyhow::Ok(())
