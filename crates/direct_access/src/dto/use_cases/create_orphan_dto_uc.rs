...
impl CreateOrphanDtoUseCase {
    ...
    pub fn execute(&mut self, dto: CreateDtoDto) -> Result<DtoDto> {
        ...
        let entity = uow.create_orphan_dto(&dto.into())?;
        uow.commit()?;
        // store in undo stack
...
impl UndoRedoCommand for CreateOrphanDtoUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_dto(&last_entity)?;
            uow.commit()?;
