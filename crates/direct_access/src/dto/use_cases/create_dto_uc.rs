...
impl CreateDtoUseCase {
    ...
    pub fn execute(&mut self, dto: CreateDtoDto, owner_id: EntityId, index: i32) -> Result<DtoDto> {
        ...
        let entity = uow.create_dto_with_owner(&dto.into(), owner_id, index)?;
...
impl UndoRedoCommand for CreateDtoUseCase {
    fn undo(&mut self) -> Result<()> {
        if let Some(ref created) = self.created_entity {
            ...
            self.old_tree_snapshot = Some(uow.snapshot_dto(&[created.id])?);

            // Remove the created entity
            uow.delete_dto(&created.id)?;
