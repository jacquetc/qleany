...
impl CreateOrphanDtoMultiUseCase {
    ...
    pub fn execute(&mut self, dtos: &[CreateDtoDto]) -> Result<Vec<DtoDto>> {
        ...
        let entities =
            uow.create_orphan_dto_multi(&dtos.iter().map(|dto| dto.into()).collect::<Vec<_>>())?;
        uow.commit()?;
        //store in undo stack
...
impl UndoRedoCommand for CreateOrphanDtoMultiUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entities) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_dto_multi(&last_entities)?;
            uow.commit()?;
