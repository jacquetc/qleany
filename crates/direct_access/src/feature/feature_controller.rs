...
use super::{
    dtos::{CreateFeatureDto, FeatureDto, UpdateFeatureDto},
...
pub fn get_relationship_many(
    ...
    let uow_factory = FeatureReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipManyUseCase::<FeatureRelationshipField, _>::new(uow_factory);
...
pub fn get_relationship_count(
    ...
    let uow_factory = FeatureReadUoWFactory::new(db_context);
    let uc =
        use_cases::GetRelationshipCountUseCase::<FeatureRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_in_range(
    ...
    let uow_factory = FeatureReadUoWFactory::new(db_context);
    let uc =
        use_cases::GetRelationshipInRangeUseCase::<FeatureRelationshipField, _>::new(uow_factory);
...
pub fn move_relationship(
    ...
    let uow_factory = FeatureWriteUoWFactory::new(db_context, event_hub);
    let mut uc =
        use_cases::UndoableMoveRelationshipUseCase::<FeatureRelationshipField, _>::new(uow_factory);
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> FeatureDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateFeatureDto = created.into();
    ...
    fn test_get_relationship_default() {
        ...
        let created = create_one(&mut ctx);
        let rel_ids =
            get_relationship(&ctx.db, &created.id, &FeatureRelationshipField::UseCases).unwrap();
    ...
    fn test_get_relationship_count_default() {
        ...
        let created = create_one(&mut ctx);
        let count =
            get_relationship_count(&ctx.db, &created.id, &FeatureRelationshipField::UseCases)
                .unwrap();
        assert_eq!(count, 0);
    }
}
