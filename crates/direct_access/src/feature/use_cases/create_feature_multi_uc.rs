...
impl CreateFeatureMultiUseCase {
    ...
    pub fn execute(
        ...
        let entities = uow.create_feature_multi_with_owner(
...
impl UndoRedoCommand for CreateFeatureMultiUseCase {
    fn undo(&mut self) -> Result<()> {
        if !self.created_entities.is_empty() {
            ...
            self.old_tree_snapshot = Some(uow.snapshot_feature(&ids_to_remove)?);

            // Remove the created entities
            uow.delete_feature_multi(&ids_to_remove)?;
