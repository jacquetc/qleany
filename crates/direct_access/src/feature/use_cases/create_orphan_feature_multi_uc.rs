...
impl CreateOrphanFeatureMultiUseCase {
    ...
    pub fn execute(&mut self, dtos: &[CreateFeatureDto]) -> Result<Vec<FeatureDto>> {
        ...
        let entities = uow
            .create_orphan_feature_multi(&dtos.iter().map(|dto| dto.into()).collect::<Vec<_>>())?;
        uow.commit()?;
        //store in undo stack
...
impl UndoRedoCommand for CreateOrphanFeatureMultiUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entities) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_feature_multi(&last_entities)?;
            uow.commit()?;
