...
impl CreateOrphansFeatureUseCase {
    ...
    pub fn execute(&mut self, dto: CreateFeatureDto) -> Result<FeatureDto> {
        ...
        let entity = uow.create_feature(&dto.into())?;
        uow.commit()?;
        // store in undo stack
...
impl UndoRedoCommand for CreateOrphansFeatureUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_feature(&last_entity)?;
            uow.commit()?;
