...
impl CreateOrphanFeatureUseCase {
    ...
    pub fn execute(&mut self, dto: CreateFeatureDto) -> Result<FeatureDto> {
        ...
        let entity = uow.create_orphan_feature(&dto.into())?;
        uow.commit()?;
        // store in undo stack
...
impl UndoRedoCommand for CreateOrphanFeatureUseCase {
    fn undo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.undo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.delete_feature(&last_entity.id)?;
            uow.commit()?;
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_feature(&last_entity)?;
            uow.commit()?;
