...
impl CreateFeatureUseCase {
    ...
    pub fn execute(
        ...
        let entity = uow.create_feature_with_owner(&dto.into(), owner_id, index)?;
...
impl UndoRedoCommand for CreateFeatureUseCase {
    fn undo(&mut self) -> Result<()> {
        if let Some(ref created) = self.created_entity {
            ...
            self.old_tree_snapshot = Some(uow.snapshot_feature(&[created.id])?);

            // Remove the created entity
            uow.delete_feature(&created.id)?;
