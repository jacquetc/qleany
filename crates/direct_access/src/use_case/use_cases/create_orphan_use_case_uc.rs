...
impl CreateOrphanUseCaseUseCase {
    ...
    pub fn execute(&mut self, dto: CreateUseCaseDto) -> Result<UseCaseDto> {
        ...
        let entity = uow.create_orphan_use_case(&dto.into())?;
        uow.commit()?;
        // store in undo stack
...
impl UndoRedoCommand for CreateOrphanUseCaseUseCase {
    ...
    fn redo(&mut self) -> Result<()> {
        if let Some(last_entity) = self.redo_stack.pop_back() {
            let mut uow = self.uow_factory.create();
            uow.begin_transaction()?;
            uow.create_orphan_use_case(&last_entity)?;
            uow.commit()?;
