...
use super::{
    ...
    units_of_work::{UseCaseUnitOfWork, UseCaseUnitOfWorkRO},
    use_cases::{
...
pub fn create(db_context: &DbContext, use_case: &CreateUseCaseDto) -> Result<UseCaseDto> {
    let mut uow = UseCaseUnitOfWork::new(&db_context);
    let mut use_case_uc = CreateUseCaseUseCase::new(Box::new(uow));
    use_case_uc.execute(use_case.clone())
}
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<UseCaseDto>> {
    let uow = UseCaseUnitOfWorkRO::new(&db_context);
    let use_case = GetUseCaseUseCase::new(Box::new(uow));
    use_case.execute(id)
}

pub fn update(db_context: &DbContext, use_case: &UseCaseDto) -> Result<UseCaseDto> {
    let mut uow = UseCaseUnitOfWork::new(&db_context);
    let mut use_case_uc = UpdateUseCaseUseCase::new(Box::new(uow));
...
pub fn remove(db_context: &DbContext, id: &EntityId) -> Result<()> {
    ...
    // delete use case
    let mut uow = UseCaseUnitOfWork::new(&db_context);
    let mut use_case = RemoveUseCaseUseCase::new(Box::new(uow));
    use_case.execute(id)?;
