...
pub fn create(db_context: &DbContext, use_case: &CreateUseCaseDto) -> Result<UseCaseDto> {
    ...
    let mut use_case_uc = CreateUseCaseUseCase::new(&mut uow);
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<UseCaseDto>> {
    ...
    let use_case = GetUseCaseUseCase::new(&uow);
    use_case.execute(id)
}

pub fn update(db_context: &DbContext, use_case: &UseCaseDto) -> Result<UseCaseDto> {
    let mut uow = UseCaseUnitOfWork::new(&db_context);
    let mut use_case_uc = UpdateUseCaseUseCase::new(&mut uow);
...
pub fn remove(db_context: &DbContext, id: &EntityId) -> Result<()> {
    ...
    // delete use case
    let mut uow = UseCaseUnitOfWork::new(&db_context);
    let mut use_case = RemoveUseCaseUseCase::new(&mut uow);
    use_case.execute(id)?;
