...
use super::{
    dtos::{CreateFileDto, FileDto, UpdateFileDto},
...
pub fn move_relationship(
    ...
    let uow_factory = FileWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::MoveRelationshipUseCase::<FileRelationshipField, _>::new(uow_factory);
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> FileDto {
        create_orphan(&ctx.db, &ctx.hub, &CreateFileDto::default()).unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateFileDto = created.into();
    ...
    fn test_create_with_owner() {
        ...
        let created = create(&ctx.db, &ctx.hub, &CreateFileDto::default(), owner_id, -1).unwrap();
        assert!(created.id > 0);
        let fetched = get(&ctx.db, &created.id).unwrap();
        assert!(fetched.is_some());
    }
    ...
    fn test_get_relationship_default() {
        ...
        let created = create_one(&mut ctx);
        let rel_ids =
            get_relationship(&ctx.db, &created.id, &FileRelationshipField::Entity).unwrap();
    ...
    fn test_get_relationship_count_default() {
        ...
        let created = create_one(&mut ctx);
        let count =
            get_relationship_count(&ctx.db, &created.id, &FileRelationshipField::Entity).unwrap();
        assert_eq!(count, 0);
    }
}
