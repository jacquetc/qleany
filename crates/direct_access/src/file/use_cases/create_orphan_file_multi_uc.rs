...
pub struct CreateOrphanFileMultiUseCase {
    uow_factory: Box<dyn FileUnitOfWorkFactoryTrait>,
}

impl CreateOrphanFileMultiUseCase {
    pub fn new(uow_factory: Box<dyn FileUnitOfWorkFactoryTrait>) -> Self {
        CreateOrphanFileMultiUseCase { uow_factory }
    }
    pub fn execute(&mut self, dtos: &[CreateFileDto]) -> Result<Vec<FileDto>> {
        ...
        let entities =
            uow.create_orphan_file_multi(&dtos.iter().map(|dto| dto.into()).collect::<Vec<_>>())?;
        uow.commit()?;
