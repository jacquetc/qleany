...
impl CommandUnitOfWork for FileWriteUoW {
...
impl use_cases::WriteUoW for FileWriteUoW {
    type Entity = File;
    ...
    fn update_with_relationships_multi(&self, entities: &[File]) -> Result<Vec<File>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_file_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::WriteRelUoW<FileRelationshipField> for FileWriteUoW {
    fn set_relationship(
        &self,
        id: &EntityId,
...
pub struct FileWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl FileWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        FileWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
...
impl use_cases::WriteUoWFactory for FileWriteUoWFactory {
    type Entity = File;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = File>> {
        Box::new(FileWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for FileWriteUoWFactory {
    type Entity = File;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = File>> {
        Box::new(FileWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<FileRelationshipField> for FileWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<FileRelationshipField>> {
        Box::new(FileWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for FileReadUoW {
...
impl use_cases::ReadUoW for FileReadUoW {
    type Entity = File;

    fn get(&self, id: &EntityId) -> Result<Option<File>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_file_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<FileRelationshipField> for FileReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &FileRelationshipField,
...
pub struct FileReadUoWFactory {
    context: DbContext,
}

impl FileReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        FileReadUoWFactory {
            context: db_context.clone(),
        }
...
impl use_cases::ReadUoWFactory for FileReadUoWFactory {
    type Entity = File;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = File>> {
        Box::new(FileReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<FileRelationshipField> for FileReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<FileRelationshipField>> {
        Box::new(FileReadUoW::new(&self.context))
    }
}
